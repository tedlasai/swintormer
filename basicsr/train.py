...
def load_resume_state(opt):
    ...
    if resume_state_path is None:
        ...
        resume_state['schedulers'][0]['base_lrs'] = [opt['train']['optim_g']['lr']]
        ...
        resume_state['optimizers'][0]['param_groups'][0]['betas'] = opt['train']['optim_g']['betas']
        resume_state['optimizers'][0]['param_groups'][0]['weight_decay'] = opt['train']['optim_g']['weight_decay']
    return resume_state
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        while train_data is not None:
            ...
            if mini_gt_size < gt_size:
                ...
                patch_size = mini_gt_size * scale
                lq = lq[:, :, x0:x0 + mini_gt_size, y0:y0 + mini_gt_size]
                gt = gt[:, :, x0:x0 + patch_size, y0:y0 + patch_size]
                # c = c[:, :, x0:x0 + mini_gt_size, y0:y0 + mini_gt_size]
